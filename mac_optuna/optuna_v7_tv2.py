...
def _load_strategies_in_worker():
    """Load TV new strategies (batches 110-114) for worker process."""
    strats = {}
    import importlib.util
    batch_dir = os.path.join(PROJECT_DIR, 'strategies_tv2_batches')
    sys.path.insert(0, batch_dir)
    for bn in range(110, 115):
        path = os.path.join(batch_dir, f'strategies_tv2_batch{bn}.py')
        if not os.path.exists(path):
            continue
        try:
            spec = importlib.util.spec_from_file_location(f'batch{bn}', path)
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            exports = getattr(mod, 'STRATEGY_EXPORT', {})
            strats.update(exports)
        except Exception as e:
            print(f"  batch{bn} load error: {e}")
    return strats
...
def main():
    ...
    progress_file = os.path.join(OUTPUT_DIR, "optuna_v7_tv2_progress.json")
