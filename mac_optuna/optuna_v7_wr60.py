...
# Timeframes to test
# ACTUALIZACIÓN SABRINA 2026-04-16: PRIORIZAR estrategias con MEJOR performance en 5m/15m
# Mantener TODOS los TFs pero asignar más trials/weight a 5m/15m
USE_TFS = ['5m', '15m', '1h', '4h', '1d']

# TF PRIORITIZATION WEIGHTS (más weight = más trials asignados)
TF_PRIORITY_WEIGHTS = {
...
def calculate_trials_by_tf(base_trials, tf, weights_dict=TF_PRIORITY_WEIGHTS):
    """
    Dado trials base y un TF, retorna cuántos trials asignar a ESE TF.
    Distribución: trials_tf = base × weight[tf] / suma(weights)

    Ejemplo: base=50 trials
      5m:   50 × 2.5 / 7.9 ≈ 16 trials
      15m:  50 × 2.0 / 7.9 ≈ 13 trials
      1h:   50 × 1.0 / 7.9 ≈ 6 trials
      4h:   50 × 0.8 / 7.9 ≈ 5 trials
      1d:   50 × 0.6 / 7.9 ≈ 3 trials
    """
    if tf not in weights_dict:
        return base_trials  # fallback si TF desconocido

    total_weight = sum(weights_dict.values())
    tf_weight = weights_dict[tf]
    trials_for_tf = max(1, int(round(base_trials * tf_weight / total_weight)))

    return trials_for_tf
        ...
        try:
            ...
            if isinstance(raw, dict):
                for sname, sinfo in raw.items():
                    if 'gen' in sinfo:
                        strats[sname] = {'gen': sinfo['gen'], 'space': sinfo['space']}
...
def main():
    ...
    # V7.7: DYNAMIC TRIALS BY TF PRIORITY WEIGHTS
    # 5m/15m reciben más trials, 1d menos
    worker_args = [
        (sn, sym, tf, calculate_trials_by_tf(n_trials, tf), multi_obj, args.journal)
        for sn, sym, tf in pending
    ]

    # Mostrar distribución de trials
    tf_trials_dist = {}
    for _, _, tf, trials, _, _ in worker_args:
        tf_trials_dist[tf] = tf_trials_dist.get(tf, 0) + trials

    print(f"\n  TF Trial Distribution (weighted by priority):")
    for tf in sorted(tf_trials_dist.keys(), key=lambda x: TF_PRIORITY_WEIGHTS.get(x, 0), reverse=True):
        pct = 100.0 * tf_trials_dist[tf] / sum(tf_trials_dist.values())
        print(f"    {tf:>4s}: {tf_trials_dist[tf]:>4d} trials ({pct:>5.1f}%)")
    print()
